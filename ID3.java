...
class ID3 {
	...
	public void train(String[][] trainingData) {
		...
		for (int i = 0; i < attributes-1; i++) {
			...
			arr_totalEntropy[i] = cal_Entropy_arr(trainingData,i);
	...
	double cal_Entropy_total(String[][] trainingData){
		int rows = trainingData.length-1;
		int[] count = new int[stringCount[attributes-1]];
	...
	double cal_Entropy_arr(String[][] trainingData, int index){
		...
		for (int i = 0; i < stringCount[target_index]; i++) {
			// sub arr is
			int[] arr_class_count = new int[stringCount[trainingData.length-1]];
			int total_arr_count = 0;
			String value = strings[target_index][i];
			for (int j = 1; j < trainingData.length; j++) {
				for (int k = 0;k< stringCount[trainingData.length-1];k++){
					if (trainingData[j][trainingData.length-1].equals(strings[trainingData.length-1][k])||trainingData[j][target_index].equals(value)) {
						arr_class_count[k]++;
						total_arr_count++;
					}
				}
